...
def solve_edNEGmodel_convergence(I_stim, alpha, t_span, stim_start, stim_end, savepath, solvers, t_steps, jacobian=False):
    for solver in solvers:   
        # Save path
        if jacobian is False:
            path = os.path.join(savepath, solver + '.h5')
            hf = h5py.File(path, 'w') 
        else:
            path = os.path.join(savepath, solver + 'j.h5')
            hf = h5py.File(path, 'w') 
